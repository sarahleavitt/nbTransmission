...
Then you can plot the final results adding the confidence intervals and average $R_t$ estimates using the plotRt function.
...
resGenNoT <- nbProbabilities(orderedPair = pairDataOrdered,
                            ...
                            label = "SNPs", l = 1,
                            n = 10, m = 1, nReps = 10)
nbResultsNoT <- merge(resGenNoT[[1]], orderedPair, by = "pairID", all = TRUE)
```
...
```{r siEstimateCI}
siParsCI <- estimateSI(nbResults, indIDVar = "individualID", timeDiffVar = "infectionDiffY", pVar = "pScaled", clustMethod = "hc_absolute", cutoff = 0.05, initialPars = c(2, 2), bootSamples = 5)
siParsCI
```
