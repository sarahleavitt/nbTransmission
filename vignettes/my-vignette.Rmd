...
```{r indToPair}
pairDataRaw <- indToPair(indData, indIDVar = "individualID", separator = "_", dateVar = "infectionDate", ordered = FALSE)
str(pairDataRaw)
```
...
```{r combine}
allProbs <- merge(resGen$probabilities, pairDataOrdered, by = "pairID", all = TRUE)
tapply(allProbs$pScaled, allProbs$transmission, summary)
```

```{r results, fig.width=7, fig.height=3}
library(ggplot2)
ggplot(data = allProbs, aes(x = pScaled)) +
...
Note the message that is printed when running this funciton without specifying rangeForAvg. In order to get a more accurate estimate of the average effective reproductive number, you only want to use the stable portion of the outbreak. You can use a plot of the Rt values over time to find this portion of the outbreak. In this case I want to exclude the first 10% and the last 20% of the outbreak.
