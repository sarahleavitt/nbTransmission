...
orTab <- (resGen$estimates
          %>% mutate(orMean = round(exp(logorMean), 2),
                     orCILB = round(exp(logorCILB), 2),
                     orCIUB = round(exp(logorCIUB), 2),
                     Value = gsub("[A-z0-9]+\\:", "", level),
                     Variable = gsub("\\:[A-z0-9+-<=>]+", "", level))
          %>% select(Variable, Value, orMean, orCILB, orCIUB)
)
```
...
ggplot(data = clustHC %>% filter(individualID.2 %in% c(32, 40)),
...
* Create a pair-level dataset from an individual dataset
