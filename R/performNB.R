...
################################################################################
# This program execultes the Bayesian classification approach
################################################################################
...
#training = training portion of the contact dataframe
#validation = the rest of the contact dataframe (excluding training pairs)
#covariates = vector of covariate names
#weighting = do you use CFS to weight the covariates (default is FALSE)
#scheme = abbreviation indicating what scheme should be used to assign covariates
#goldStandard = the variable name (in quotes) that will define linking status
#threshold = theshold for SNP distances - both for defining snpClose and starting contact groups


#### Outputs ####

#List containing two dataframes
  #1: results with the predicted probabilities for each pair
  #2: messages with error or warning messages for each approach


performBayes <- function(training, validation, covariates, scheme,
                         goldStandard, weighting=FALSE){
  
  #### Create label ####
  
  if (weighting == FALSE){label <- paste0(goldStandard, "NB")}
  if (weighting == TRUE){label <- paste0(goldStandard, "NBDFW")}

  if(!is.null(scheme)){
    label <- paste0(label, scheme)
  }
  ...
  tryCatch({
    ...
    messages <- cbind(label, messages = NA)
    
    return(list(results, messages, coeff))
    ...
    results2 <- (validation
                 %>% mutate(p = NA,
                            nLinksTrain = sum(training$linked == TRUE),
                            label = label)
                 %>% select(label, edgeID, p, nLinksTrain)
    )
    messages2 <- cbind(label, messages = paste("Error:", err[[1]]))
    coeff2 <- NA
    
    return(list(results2, messages2, coeff2))
  })
