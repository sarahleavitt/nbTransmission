...
#' described in indData with SNP distance between each pair. Genomes were
...
#' \item{X2}{Pair-level covariate derived from X2: 1 if match, 0 if not match}
#' \item{X3}{Pair-level covariate derived from X3: 1 if a-a, 2 if b-b, 3 if a-b, 4 if b-a}
#' \item{X4}{Pair-level covariate derived from X4: 3 if match, 2 if adjacent, 1 otherwise}
#' }
"pairData"
